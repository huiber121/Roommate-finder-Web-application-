...
def get_all_roommates():
    ...
    if 'type' in rjson.keys():
      sql = get_sql(rjson,rjson['type'])
      LOGGER.info(' SQL statement: {}'.format(sql))
      result = dbinstance.get_data(sql) 
      result_json.append(get_roommate_json(result, rjson['type']))
    else: 
      ...
      if len(rjson) == 0:
        ...
        LOGGER.info(rjson)
        reqvalues = rjson.values()
        for x in data:
          for xval in x:
            checkval =  all(item in xval.values() for item in rjson.values())
            if checkval == True:
              result_json.append(xval)      
     

    LOGGER.info(' JSON before returning: {} '.format(len(result_json)))
...
def get_sql(uijson, usertype):
  ...
  if usertype == 'student':
        ...
        if 'age' in uijson.keys():
          check_param = check_param+1
          LOGGER.info(check_param, param_len)
          if(check_param < param_len):
