...
def add_school(uijson):
    ...
    if len(schooexists) == 0:
        ...
        LOGGER.info("Before add school result {%s}",result)
        return sid
    else:
        LOGGER.info("School exists {%s} ", schooexists[0][0])
...
def add_student(uijson,uid,schoolid):
    ...
    LOGGER.info("After adding in student_user {%s}",result)
    return result
